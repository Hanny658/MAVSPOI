...
def profiler_system_prompt() -> str:
    return (
        ...
        "You are additionally provided tool summaries (freq/cat/time/loc/poi) that mimic trajectory statistics.\n"
        "Rules:\n"
        "- Use only provided input facts.\n"
        "- Use tool summaries to infer stable and recent behavior patterns.\n"
...
def forecaster_system_prompt() -> str:
    return (
        ...
        "- merged candidate set for Predictor\n"
        "Rules:\n"
        "- business_id must come from provided candidates only.\n"
        "- Use `short_term_initial_candidate_ids` as the primary CC initializer, then refine.\n"
        "- Respect hard_constraints and prioritize keeping likely true POIs in CH/CC.\n"
...
def predictor_system_prompt() -> str:
    return (
        ...
        "Rules:\n"
        "- Primary selection should be from provided final candidates.\n"
        "- If confidence is low, you may choose from `candidate_universe` outside final candidates.\n"
        ...
        "- Read input field `top_k` and output exactly `top_k` recommendation items.\n"
        "- Do not output fewer items unless there are fewer than `top_k` valid candidates.\n"
